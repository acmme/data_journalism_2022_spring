...
# Write the code to produce your answer here
west_virginia_ppp %>%
  select(servicing_lender_name, undisbursed_amount) %>%
  mutate (
    positive_undisbursed_amount = case_when(
      undisbursed_amount > 0 ~ 'undisbursed',
      undisbursed_amount == 0 ~ 'disbursed'
    )
  ) %>%
  filter(positive_undisbursed_amount == 'undisbursed') %>%
  arrange(servicing_lender_name, positive_undisbursed_amount, desc(undisbursed_amount))

```

**Q3** What are some potential reasons why a loan might not be paid out? What questions does that circumstance raise for you?
**A3** [They could have been approved but later found fraudulent. They could have also been made fairly recently, and so it hasn't been immediately disbursed.]
...
**A5** [10 loans went to West Virginia applicants that listed the maximum number of jobs retained.J. Austin Heating & Cooling, Inc. had the earliest approved date.]
