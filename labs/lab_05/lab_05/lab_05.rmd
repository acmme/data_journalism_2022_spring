...
**A1.** The street address that has the most loans is 126 E Burke St, with 6 loans. The names of the businesses at that address are: Drew Holdings LLC; Brix27, LLC; Abraham Ashton; Hub Co-op, LLC; and Ronin Properties LLC.

```{r}
loan_sample <- read_csv("data/zip_25401_loan_sample_clean.csv")
...
**A2.** This address is a community workspace called The Hub for local businesses to rent out. Abraham Ashton, however, is listed as the organizer of the realtors who supposedly operates here, Drew Holdings and Ronin Properties, and as the organizer of a restaurant, Brix 27. While it might be plausible that the restaurant would rent out office space here, it's not clear whether Abraham Ashton actually owns these businesses, and if he did, whether he would need loans for these businesses AND as a self-employed person.
...
write_csv(wv_ppp, "data/wv_ppp.csv")
...
**A3.** Martinsburg had the highest number of loans in West Virginia's second largest county. In Martinsburg, Full-service restaurants had more loans than any other industry title.
...
wv_ppp <- read_csv("data/wv_ppp_clean.csv")
wv_ppp_with_naics <- wv_ppp %>%
  left_join(naics)
...
wv_ppp_with_naics %>% 
  filter(project_county_name == "BERKELEY" & city == "Martinsburg") %>% 
  group_by(title) %>% 
  summarize(
    count = n()
  ) %>% 
...
**A4.** One hypothesis is that restaurants tend to already be at risk of failure as an industry, and so they would be put most at risk in the event of a crisis. Another hypothesis is that Martinsburg has a high concentration of restaurants, and so the loans might be representative of the businesses in the area.
...
**A5.** Cottle Christi L LLC  got the highest approved loan amount, for $280,434. The data said the company retained 94 jobs. The restaurant is known as Kitzie's, and currently has 2 locations. One of its locations, Inwood, closed. 
```{r}
martinsburg_restaurants <- wv_ppp_with_naics %>% 
  filter(city == "Martinsburg" & title == "Full-Service Restaurants")

martinsburg_restaurants %>% 
  group_by(name, jobs_retained, current_approval_amount) %>%
  summarize(
    count = n()
  ) %>% 
  arrange(count, desc(current_approval_amount), jobs_retained)
```
