...
**Answer**: There are 192,959 loans where franchise_name is NA. There are 275 loans where franchise_name is "Subway."
...
**Answer**: There are 135,073 loans where the forgiveness_amount is NA.
...
**Answer**: There are 135,073 loans where forgiveness_date is NA, the same number as for forgiveness_amount.
...
**Answer**: There are 135,073 loans where forgiveness_amount and forgiveness_date are NA, the same number as for forgiveness_amount and forgiveness_date run separately. This means that there are 135,073 loans that either were not forgiven or not recorded as forgiven.
...
**Answer** There are no loans for October 2020.
```{r}
ppp_maryland_loans %>%
...
**Answer** The largest loan is for $10,000,000. The smallest loan is for 6 dollars. The minimum amount is suspicious since there is no reason for anyone to need just 6 dollars for payroll, let alone utilities, rent, or other loan needs.
```{r}
...
**Answer** There are two loans under $100.
```{r}
...
**Answer** They align for multiple fields.
...
**Answer** There are 6 rows in this demonstration data set.
...
**Answer** The "amount" field is the data type <chr>.
```{r}
...
**Answer** The street address and city data are spelled with different capitalization rules. There are two mispelled cities. The zip code data range in specificity. There is one row that is doubled.
...
**Answer** The function changed 1_id to x1_id.
...
The summarise function can't sum a column with a <chr> datatype.
...
**Answer** The new data type is <dbl> for amount.
...
**Answer** The total amount of all loans is $2,033,179
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** There are two duplicate rows.
...
**Answer** The new dataframe has 5 rows.
...
**Answer** Two misspellings remain.
...
**Answer** One misspelling of Arnold remains.
