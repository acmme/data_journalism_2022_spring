...
**Answer** A dataframe that shows which 2016 candidates won the majority in which states, what percent of voters in each state voted for Trump, and what percent of businesses in those states received PPE during the pandemic.
```{r}
...
**Answer** The plot shows a trend in which the more there were people who voted for Trump in 2016 in each state, the more businesses in that state received PPE.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** The plot shows less of a correlation between the percent of Trump voters in a state and the amount of PPE distributed per employee.
```{r}
...
**Answer** The statistical significance of the percent of Trump voters in a state and the percent of businesses receiving PPE and the amount of PPE distributed per employee.
```{r}
reveal_data %>%
...
**Answer** A correlation test was run between the percent of Trump voters and the percent of businesses receiving PPE, and determines that the correlation is statistically significant, if moderately correlated.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** A correlation test was run between the percent of Trump voters and the amount of PPE distributed per employee, and determines that the correlation is statistically insignificant, and not well-correlated.
```{r}
...
**Answer** The table compares the average state that voted for Clinton and the average state that voted for Trump in 2016, by the average PPE distributed per employyee and the average percent of businesses that received PPE in those states.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
    mean_ppe_amount_per_employee = mean(ppe_amount_per_employee),
...
**Answer** A t-test that determines the statistical significance in the difference between states that voted for Clinton and states that voted for Trump in terms of the PPE distributed per employee in those states. The p-value is high, showing that the difference between the states is not statistically significant.
```{r}
...
### Task 10: Run t-test
**Task** Run the  following code. Describe the output. What do you see?
**Answer** A t-test that determines the statistical significance in the difference between states that voted for Clinton and states that voted for Trump in terms of the percent of businesses that received PPE in those states. The p-value is low enough to show that the difference between the states is statistically significant.
```{r}
